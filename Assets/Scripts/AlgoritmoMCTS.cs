...
public class AlgoritmoMcts
{
    public Ficha Mcts(GameManager gameManager)
    {
        ...
        // sean necesarias
        RunMCTS(root, gameManager, 6, root);

        var visits = 0;
        Ficha move = new Ficha();
        foreach (var finalMove in root.children)
        {
            Debug.Log("Para ver la cantidad de veces que se visitó cada nodo");
            Debug.Log(JsonUtility.ToJson(finalMove));
    ...
    public NodoMCTS Selection(NodoMCTS nodo)
    {
        ...
        // Aqí hay una posibilidad de devolver un nodo inicializado, pero sin ningún
